function init() {
  ...
  function buildHeatMap() {
    return (
      d3.json("script/state.json").then(function (json) {
        svg.selectAll("path")
          .data(json.features)
          .enter()
          .append("path")
          ...
          .on("click", function () {
            ...
            var d = d3.select(this).data()[0];
            var choosenState = d.properties.STATE_NAME;

            //Import csv
            d3.csv("csv/stateMigration.csv", function (d) {
              return {
                country: d.Country,
                state: +d[choosenState]    //only imported the data for selected state
              }
            }).then(function (data) {
              stateData = data;
              ...
              d3.json("script/world.json").then(function (json) {
                ...
                var usedCountries = ["GBR", "NZL", "CHN", "IND",
                  "PHL", "VNM", "ITA", "ZAF",
                  "MYS", "LKA", "GRC", "KOR",
                  "USA", "IDN", "NLD"];

                // Filter the features based on the selected country codes
                var checkedCountries = excludingVar.filter(function (d) {
                  return usedCountries.includes(d.properties.ISO_A3);
                });
                ...
                d3.csv("csv/countryCentre.csv", function (centre) {
                  ...
                  for (var i = 0; i < centreData.length; i++) {
                    var source = [australiaY, australiaX];      //Hard coded centre of Australia
